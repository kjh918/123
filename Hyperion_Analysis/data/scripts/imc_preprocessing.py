...
for fol in folders:
    for fn in os.listdir(fol):
        if re_fn.match(fn):
            ...
            print(fn_full)
            try:
                convertfolder2imcfolder.convert_folder2imcfolder(fn_full, \
                out_folder=folder_ome, adozip=False)
            except:
    ...
    for img in os.listdir(sub_fol):
        ...
        for (col, suffix, addsum) in list_analysis_stacks:
            try:
                ...
                folder_analysis, basename + suffix, pannelcsv=csv_pannel, \
                metalcolumn=csv_pannel_metal,usedcolumn=col, addsum=addsum, \
                bigtiff=False,pixeltype='uint16')
            except:
